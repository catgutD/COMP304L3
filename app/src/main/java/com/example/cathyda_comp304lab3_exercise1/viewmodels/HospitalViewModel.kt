...
class HospitalViewModel(private val hospitalDao: HospitalDao): ViewModel() {
    ...
    fun insertPatient(patientEntity: PatientEntity) = hospitalDao.insertNewPatient(patientEntity)

    fun insertTest(testEntity: TestEntity) = hospitalDao.insertNewTest(testEntity)

    fun insertNurse(nurseEntity: NurseEntity) = hospitalDao.insertNewNurse(nurseEntity)

    fun updatePatient(patientEntity: PatientEntity) = hospitalDao.updatePatient(patientEntity)
